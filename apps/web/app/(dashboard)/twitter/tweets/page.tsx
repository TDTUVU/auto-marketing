...
export default async function TweetsPage() {
  await connectDB()
  const twitterAccounts = await Account.find({ platform: 'twitter' }).select('_id name').lean()
  const accountIds = twitterAccounts.map((a) => a._id)
  const accountMap = Object.fromEntries(twitterAccounts.map((a) => [a._id.toString(), a.name]))

  const posts = await Post.find({ accountId: { $in: accountIds } })
    .sort({ createdAt: -1 })
    .limit(50)
    .lean()
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-semibold text-zinc-900">Tweets</h1>
          <p className="text-sm text-zinc-500 mt-0.5">{posts.length} tweet tổng cộng</p>
        </div>
        <Link href="/dashboard/twitter/tweets/new">
          <Button>
      ...
      {posts.length === 0 ? (
        <div className="text-center py-16 text-zinc-400">
          ...
          <p className="text-sm">Chưa có tweet nào</p>
          <Link href="/dashboard/twitter/tweets/new" className="mt-3 inline-block">
            <Button variant="outline" size="sm">Tạo tweet đầu tiên</Button>
          </Link>
        </div>
      ) : (
                    ...
                    {post.imageUrls.length > 0 && (
                      <div className="flex gap-1.5 mt-2.5">
                        {post.imageUrls.slice(0, 4).map((img: string, i: number) => (
                          <img key={i} src={`/api/uploads/${img}`} alt="" className="size-14 rounded-lg object-cover border border-zinc-200" />
                        ))}
