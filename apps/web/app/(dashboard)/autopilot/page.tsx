...
export default async function AutoPilotPage() {
  ...
  for (const c of configs) {
    configMap[c.accountId.toString()] = {
      ...
      postTimes: c.postTimes,
      tone: c.tone,
      minIntervalDays: c.minIntervalDays,
      categories: c.categories,
      autoReplyEnabled: c.autoReplyEnabled,
      replyTone: c.replyTone,
      skipSpam: c.skipSpam,
    }
  }
    ...
    <div className="p-6 max-w-2xl mx-auto">
      <div className="mb-6">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-xl font-semibold text-zinc-900">Auto-pilot</h1>
        </div>
        <p className="text-sm text-zinc-500 mt-1">
          Bật auto-pilot để hệ thống tự tạo bài từ catalog sản phẩm và đăng theo lịch.
          Không cần lên lịch thủ công — chỉ cần thêm sản phẩm vào catalog.
        </p>
      </div>

      {accounts.length === 0 ? (
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 text-sm text-amber-800">
          Chưa có tài khoản nào. Thêm tài khoản trước.
        </div>
      ) : (
        <AutoPilotConfigForm
